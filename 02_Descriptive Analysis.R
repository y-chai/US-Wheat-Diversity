...
# Load data
WVA.data <- fread("./data_original/GEMS_WVA_data_v1.0.csv")
MV.list <- fread("./data_original/GEMS_MV.list_v1.0.csv")
...
# Save variety names
var.anon <- data.frame(var_std = var.tN.sel$var_std, var_anon = var.tN.sel$var_std)

var.anon <- rbind(var.anon, data.frame(var_std = var.m20N.sel$var_std, var_anon = paste0("VAR-OTHER-", seq(1:nrow(var.m20N.sel)))))
var.anon <- rbind(var.anon, data.frame(var_std = var.bN.sel$var_std, var_anon = paste0("VAR-OTHER-", nrow(var.m20N.sel) + seq(1:nrow(var.bN.sel)))))
                  
write.csv(var.tN.sel, file="./data_intermediate/02_var.tN.sel.csv", row.names = F)
write.csv(var.m20N.sel, file="./data_intermediate/02_var.m20N.sel.csv", row.names = F)
write.csv(var.bN.sel, file="./data_intermediate/02_var.bN.sel.csv", row.names = F)
write.csv(var.anon, file="./data_intermediate/02_var_anon.csv", row.names = F)
...
# State List by wheat classes (manually selected based on the "summary_NoV_st_class.csv")
