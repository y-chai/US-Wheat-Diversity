...
# Save figure
png("./plot/PD1_Decomp_DSW_yr.png", width = 10, height = 10, units = "in", res= 300)
...
write.csv(PD1.Decom.DSW.st, file="./Fin Data/04_03_PD1.Decom.DSW.st.csv")
...
# Save figure
png("./plot/PD1_Decomp_DSW_st.png", width=10, height=10, units="in", res=300)

ggplot(data=PD1.Decom.DSW.st) + 
  ...
  scale_x_continuous(name=expression(paste(alpha, " diversity (effective varieties per epoch)")), breaks = c(seq(0, 12, 2))) +
  scale_color_manual(name="Market Class", values=c("#e41a1c", "#4daf4a", "#377eb8"),
                     labels=c("Durum", "Spring", "Winter")) +
