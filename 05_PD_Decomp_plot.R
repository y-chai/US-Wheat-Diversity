...
load("./data_intermediate/PD_Durum_by_year_abr.RData")
PD.Decom.Durum.yr.abr <- D_yr.abr
load("./data_intermediate/PD_Spring_by_year_abr.RData")
PD.Decom.Spring.yr.abr <- D_yr.abr
load("./data_intermediate/PD_Winter_by_year_abr.RData")
...
# Save figure
png("./plot/05_PD1_Decomp_DSW_yr.png", width = 8, height = 10, units = "in", res= 300)
...
load("./data_intermediate/PD_Durum_by_state_abr.RData")
PD.Decom.Durum.st.abr <- D_st.abr

load("./data_intermediate/PD_Spring_by_state_abr.RData")
PD.Decom.Spring.st.abr <- D_st.abr

load("./data_intermediate/PD_Winter_by_state_abr.RData")
...
write.csv(PD1.Decom.DSW.st, file="./data_final/04_03_PD1.Decom.DSW.st.csv")
...
# my_grid <- my_grid[(my_grid$name %in% unique(PD1.Decom.DSW.st$state)), ]
# # grid_preview(my_grid)
# 
# my_grid <- as.data.table(my_grid)

# write.csv(my_grid, "./data_original/my_us_state_grid.csv", row.names = F)
...
plt_PD1_Decomp_DSW_st <- ggplot(data=PD1.Decom.DSW.st) + 
  ...
  scale_x_continuous(name=expression(paste("State-level ", alpha, " diversity (effective varieties per epoch)")), breaks = c(seq(0, 12, 2))) +
  scale_color_manual(name="Market Class", values=c("#e41a1c", "#4daf4a", "#377eb8"),
                     labels=c("Durum", "Spring", "Winter")) +
...
save(plt_PD1_Decomp_DSW_st, file="./data_intermediate/plt_PD1_Decomp_DSW_st.RData")


png("./plot/05_PD1_Decomp_DSW_st.png", width=8, height=16, units="in", res=300)

plt_PD1_Decomp_DSW_st 
